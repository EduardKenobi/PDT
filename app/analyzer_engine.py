...
def process_open_positions(open_positions: pd.DataFrame, current_price: float, price_currency: str, exchange_rate_cache: Dict) -> Tuple[List[PositionData], float, float, float, Dict]:
    ...
    for _, pos in open_positions.iterrows():
        ...
        if current_price and price_currency:
            rate_now = 1 if price_currency == PRIMARY_CURRENCY else get_rate_from_cache(exchange_rate_cache, price_currency, PRIMARY_CURRENCY, today_str)
            if rate_now:
                current_lot_value_primary = (pos['shares'] * current_price) * rate_now
            elif price_currency != PRIMARY_CURRENCY:
...
def calculate_ticker_metrics(ticker, transactions_df, dividends_data, ticker_map_data, market_data) -> Optional[TickerData]:
    ...
    if price_currency and price_currency != PRIMARY_CURRENCY:
        today_str = datetime.now().strftime('%Y-%m-%d')
        rate = get_rate_from_cache(exchange_rate_cache, price_currency, PRIMARY_CURRENCY, today_str)
        if rate:
            padi *= rate
